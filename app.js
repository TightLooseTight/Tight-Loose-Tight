...
Dine svar er anonyme og det er kun du som kan se dine score/feedback.`
    ...
    badges:{ strong:"Sterk", balanced:"Middels", weak:"Svak" },
...
Dine svar er anonyme, og kun du kan se dine scorer/feedback.`
    ...
    reset:"Nulstil",
    resultsTitle:(n)=>`Her er dine resultater${n?` (${n})`:''}`,
    badges:{ strong:"Stærk", balanced:"Mellom", weak:"Svag" },
...
Dina svar är anonyma och bara du kan se dina poäng/feedback.`
    ...
    badges:{ strong:"Stark", balanced:"Medel", weak:"Svag" },
...
Praktisk tips:
Fortsett dialogen med teamet rundt kontinuerlig forbedring, utforskning og samhandling. Inviter andre team til å lære av deres erfaringer.`
    }
  },
...
let st = {
  lang:'no',
  name:'',
  answers:{ t1:Array(5).fill(null), loose:Array(5).fill(null), t2:Array(5).fill(null) },
  expanded:{t1:false,loose:false,t2:false}
};
...
function load(){
  ...
  Object.entries(st.answers).forEach(([k,arr])=>arr.forEach((v,i)=>{
    if(v){
      const el=document.querySelector(`.pill[data-q="${k}-${i}"][data-v="${v}"]`);
...
function showPage(i){
  document.querySelectorAll('.page').forEach((p,idx)=>
    p.classList.toggle('hidden', idx!==i-1)
  );
}

// ---- Flag clicks / set active ----
document.addEventListener('click', (e)=>{
  if(e.target && e.target.id==='flag-no'){
    st.lang='no'; save(); applyLang();
    $('#flag-no')?.classList.add('active');
    $('#flag-dk')?.classList.remove('active');
    $('#flag-se')?.classList.remove('active');
  }
  if(e.target && e.target.id==='flag-dk'){
    st.lang='dk'; save(); applyLang();
    $('#flag-dk')?.classList.add('active');
    $('#flag-no')?.classList.remove('active');
    $('#flag-se')?.classList.remove('active');
  }
  if(e.target && e.target.id==='flag-se'){
    st.lang='se'; save(); applyLang();
    $('#flag-se')?.classList.add('active');
    $('#flag-no')?.classList.remove('active');
    $('#flag-dk')?.classList.remove('active');
  }
});
...
function updateScoreboxes(hideQuestions){
  ...
  [['t1',s1],['loose',sL],['t2',s2]].forEach(([k,s])=>{
    const sb=document.getElementById('sb-'+k),
          sbb=document.getElementById('sbb-'+k),
          sbv=document.getElementById('sbv-'+k);
    if(sb && sbb && sbv){
      if(s>0){
        sb.classList.remove('disabled');
        const b=badge25(s, st.lang);
        sbb.innerHTML=`<span class="badge ${b.cls}">${b.t}</span>`;
        sbv.textContent=`${s}/25`;
      } else {
  ...
  if(hideQuestions){
    st.expanded={t1:false,loose:false,t2:false};
    save();
    renderAccordion();
  }
  fillResultsPage();
...
function fillResultsPage(){
  const scores = {
    t1: sum(st.answers.t1),
    loose: sum(st.answers.loose),
    t2: sum(st.answers.t2)
  };
  const inline=$('#userNameInline');
  if(inline) inline.textContent = st.name || '';

  Object.entries(scores).forEach(([k,s])=>{
    const r=document.getElementById('r-'+k),
          rv=document.getElementById('rv-'+k),
          rb=document.getElementById('rb-'+k),
          rt=document.getElementById('rt-'+k);
    if(r && rv && rb && rt){
      if(s>0){
  ...
  if (any && svg){
    try { drawRadar(scores); }
      ...
      console.warn('Radar disabled due to error:', e);
...
function wireButtons(){
  $('#btn-calc')?.addEventListener('click', ()=>{
    updateScoreboxes(true);
    const t = (st.lang==='dk')
      ? 'Resultater opdateret'
      : (st.lang==='se' ? 'Resultat uppdaterat' : 'Resultater oppdatert');
    toast(t);
  });
